(function () {
  ...
  ns.FramesListController = function (piskelController, container) {
    ...
    this.cachedFrameProcessor.setFrameProcessor(
      this.frameToPreviewCanvas_.bind(this)
    );
    this.cachedFrameProcessor.setOutputCloner(
      this.clonePreviewCanvas_.bind(this)
    );
  ...
  ns.FramesListController.prototype.init = function () {
    ...
    this.previewListScroller.addEventListener(
      "scroll",
      this.updateScrollerOverflows.bind(this)
    );
  ...
  ns.FramesListController.prototype.updatePreviews_ = function () {
      ...
      if (this.tiles[i].getAttribute("data-tile-hash") !== hash) {
        if (this.tiles[i].querySelector("canvas")) {
          this.tiles[i]
            ...
            .replaceChild(
              this.getCanvasForFrame(
                this.piskelController.getCurrentLayer().getFrameAt(i)
              ),
    ...
    // Add selected class
    this.tiles[this.piskelController.getCurrentFrameIndex()].classList.add(
      "selected"
    );
  };
